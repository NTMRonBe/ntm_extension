...
class regional_upload(osv.osv_memory):
    ...
    def uploadData(self, cr, uid, ids, context=None):
        for form in self.read(cr, uid, ids, context=None):
            periodReader = self.pool.get('account.period').read(cr, uid, form['period_id'],['date_start','date_stop'])
            move = {
            ...
            for trans_id in context['active_ids']:
                transactionReader = self.pool.get('regional.uploader').read(cr, uid, trans_id, context=None)
                curr_id = False
                account_id = False
                analytic_id = False
                checkAccpacAccount = self.pool.get('account.accpac').search(cr, uid, [('name','=',transactionReader['acctid'])])
                accpac_acc = transactionReader['acctid']
                if not checkAccpacAccount:
                    accSearchNormal = self.pool.get('account.account').search(cr, uid, [('code','=',transactionReader['acctid'])], limit=1)
                    if not accSearchNormal:
                        accSearchAnalytic = self.pool.get('account.analytic.account').search(cr, uid, [('code','=',transactionReader['acctid'])], limit=1)
                        if not accSearchAnalytic:
                            raise osv.except_osv(_('Error !'), _('RU-001: Account %s is not on the list of any accounts!')%accpac_acc)
                ...
                elif checkAccpacAccount:
                    acc_read = self.pool.get('account.accpac').read(cr, uid, checkAccpacAccount[0], context=None)
                    if acc_read['account_id']:
                        ...
                        account_id = analyticRead['normal_account'][0]
                curr_search = self.pool.get('res.currency').search(cr, uid, [('name','ilike',transactionReader['currency'])])
                for currency in curr_search:
                    curr_id = currency
                currency_read = self.pool.get('res.currency').read(cr, uid, curr_id, ['rate'])
                amount = transactionReader['scuramt'] / currency_read['rate']
                credit = 0.00
                debit = 0.00
                if amount > 0.00:
                    credit = 0.00
                    ...
                    credit = amount * -1
                    debit = 0.00
                if transactionReader['date']<periodReader['date_start'] or transactionReader['date']>periodReader['date_stop']:
                    raise osv.except_osv(_('Error !'), _('RU-002: The date of the transaction is not within the period!'))
                else:
                    name = transactionReader['transdesc'] +' '+transactionReader['date']
                    move_line = {
                        'name':name,
                        'journal_id':form['journal_id'],
                        'period_id':form['period_id'],
                        ...
                        'date':transactionReader['date'],
                        'ref':transactionReader['ref'],
                        'move_id':move_id,
                        'amount_currency':transactionReader['scuramt'],
                        'currency_id':curr_id,
                        }
                    self.pool.get('account.move.line').create(cr, uid, move_line)
        return {'type': 'ir.actions.act_window_close'}

    def upload2(self, cr, uid, ids, context=None):
