...
class account_analytic_statement_parser(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(account_analytic_statement_parser, self).__init__(cr, uid, name, context=context)
    ...
    def _get_partner_name(self, partner_id):
        self.cr.execute("select name from res_partner where id=%s",(partner_id))
        return self.cr.fetchone()[0] or 0.00
...
report_sxw.report_sxw('report.account.analytic.account.statement', 'account.analytic.account', 'addons/ntm_extension/report/statement.rml',parser=account_analytic_statement_parser, header="internal")
